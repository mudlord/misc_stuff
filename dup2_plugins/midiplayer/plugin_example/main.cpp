...
const char* __stdcall  DUP2_ModuleDescription(MY_PLUGIN_DATA_STRUCTURE* plugin_data) {
	return "Plays back MIDI sequences";
}
...
int  DialogPlugin(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
	switch ( message ) {
		case WM_INITDIALOG:
			...
			LoadWindowPosition(hwnd, "com.mudlord.midi.player");
			LoadPluginData();
			break;
		...
		case WM_CLOSE:
			EndChooseHideMethod();
			SaveWindowPosition(hwnd, "com.mudlord.midi.player", POS_NOSIZE);
			EndDialog(hwnd, 0);
			break;
