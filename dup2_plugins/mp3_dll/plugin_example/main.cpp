...
const char* __stdcall  DUP2_ModuleDescription(MY_PLUGIN_DATA_STRUCTURE* plugin_data) {
	return "Plays back MP3/OGG streams";
}
...
int  DialogPlugin(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
	switch ( message ) {
		case WM_INITDIALOG:
			...
			LoadWindowPosition(hwnd, "com.mudlord.mp3.player");
			LoadPluginData();
			break;
			...
			switch ( LOWORD(wparam) ) {
				...
				case BTN_LOAD:
					if ( d2k2_GetFilePath(local_buffer,
					                      "Music files [mp3,ogg]\0*.mp3;*.ogg\0",
					                      "c:\\",
					                      hDialogPlugin) ) {
		...
		case WM_CLOSE:
			EndChooseHideMethod();
			SaveWindowPosition(hwnd, "com.mudlord.mp3.player", POS_NOSIZE);
			EndDialog(hwnd, 0);
			break;
