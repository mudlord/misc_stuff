...
const char* __stdcall  DUP2_ModuleDescription(MY_PLUGIN_DATA_STRUCTURE* plugin_data) {
	return "Plays back NSF/NSFE files";
}
...
int  DialogPlugin(HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam) {
	switch ( message ) {
		case WM_INITDIALOG:
			...
			LoadWindowPosition(hwnd, "com.mudlord.nsf.player");
			LoadPluginData();
			break;
			...
			switch ( LOWORD(wparam) ) {
				...
				case BTN_LOAD:
					if ( d2k2_GetFilePath(local_buffer,
					                      "NES music files [nsf,nsfe]\0*.nsf;*.nsfe\0",
					                      "c:\\",
					                      hDialogPlugin) ) {
		...
		case WM_CLOSE:
			EndChooseHideMethod();
			SaveWindowPosition(hwnd, "com.mudlord.nsf.player", POS_NOSIZE);
			EndDialog(hwnd, 0);
			break;
