...
static mal_uint32 audio_callback(mal_device* pDevice, mal_uint32 frameCount, void* pSamples)
{
	...
	int ret =  amount_buf((uint8_t*)pSamples, frameCount) / mal_get_bytes_per_frame(mal_format_f32, pDevice->channels);
...
bool audio_init(double refreshra, float input_srate, float fps)
{
	...
	if (mal_device_init(NULL, mal_device_type_playback, NULL, &config, &audio_ctx_s, &audio_ctx_s.device) != MAL_SUCCESS) {
		return false;
	}
	...
	size_t sampsize = (mal_device_get_buffer_size_in_bytes(&audio_ctx_s.device)*4);
	audio_ctx_s._fifo = fifo_new(sampsize); //number of bytes
	
	audio_ctx_s.output_float = new float[outsamples_max]; //spare space for resampler
	audio_ctx_s.input_float = new float[outsamples_max];
	if (mal_device_start(&audio_ctx_s.device) != MAL_SUCCESS)return false;
...
void audio_mix(const int16_t* samples, size_t size)
{
	...
	auto bufferlevel = []() {return double((audio_ctx_s._fifo->size - (int)fifo_write_avail(audio_ctx_s._fifo)) / audio_ctx_s._fifo->size); };
	int newInputFrequency = ((1.0 - maxdelta) + 2.0 * (double)bufferlevel() * maxdelta) * audio_ctx_s.system_rate;
	float drc_ratio = (float)audio_ctx_s.client_rate / (float)newInputFrequency;
	mal_pcm_s16_to_f32(audio_ctx_s.input_float, samples, in_len, mal_dither_mode_triangle);
	...
	while (written < out_len)
	{
		...
		if (avail)
		{
			...
			fifo_write(audio_ctx_s._fifo, (const char*)audio_ctx_s.output_float + written, write_amt);
			written += write_amt;
