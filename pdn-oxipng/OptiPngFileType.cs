...
namespace ILikePi.FileTypes.OptiPng
{
    ...
    internal class OptiPngFileType : FileType<OptiPngSaveConfigToken, OptiPngSaveConfigWidget>
    {
        ...
        private static readonly string OptiPNGPath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "oxipng.exe");
...
#endif
        {
            ...
            {
                throw new FileNotFoundException("Please ensure you've installed oxi.exe into the FileTypes folder.", "oxipng.exe");
            }

            string path = Path.Combine(Path.GetTempPath(), "PDN_OxiPNG_");
        ...
        protected override void OnSaveT(Document input, Stream output, OptiPngSaveConfigToken token, Surface scratchSurface, ProgressEventHandler progressCallback)
        {
            ...
            if (!token.Equals(tempFileToken) || !areSurfacesEqual(scratchSurface, tempFileSurface))
            {
                ...
                if (token.Optimize)
                {
                    string args = $"\"{tempFile}\" -o{token.Compression} -t{(token.Interlace ? "1" : "0")} -t{token.ThreadCount} {(token.Quiet ? " -quiet" : string.Empty)}" +
                        $"{(token.Zopfli ? " -Z" : string.Empty)} { (token.LibDeflater ? " -D" : string.Empty)}";
